...
impl PubRec {
    ...
    pub(super) fn read_traced(
        ...
        let properties = PubRecProperties::read_traced(&mut bytes, order)?;
        let puback = PubRec {
            pkid,
            reason: reason(ack_reason)?,
            properties,
...
impl PubRecProperties {
    ...
    pub(super) fn read_traced(
        bytes: &mut Bytes,
        order: &mut Vec<u8>,
    ) -> Result<Option<PubRecProperties>, Error> {
...
impl From<PubRecReason> for u8 {
    fn from(value: PubRecReason) -> Self {
        code(value)
    }
}
